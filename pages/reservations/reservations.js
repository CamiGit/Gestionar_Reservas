...
function renderizarReservas() {
  ...
  if (!servicio) {
    container.innerHTML =
      "<p class='text-muted text-center'>No hay ningún servicio seleccionado aún.</p>";
    return;
  }
...
function initFechaHora(estilista) {
  ...
  estado.fecha = null;
  estado.hora = null;
  estado.idServicio = JSON.parse(localStorage.getItem("servicioSeleccionado"))?.id;
...
function renderCalendario() {
  ...
  const diasDisponibles = new Set(Object.keys(estado.estilista?.fechas || {}));
  ...
  for (let dia = 1; dia <= diasDelMes; dia++) {
    const mesFormateado = String(estado.mes + 1).padStart(2, "0");
    const diaFormateado = String(dia).padStart(2, "0");
    const fecha = `${estado.anio}-${mesFormateado}-${diaFormateado}`;

    const hoy = new Date();
    hoy.setHours(0, 0, 0, 0);
    const fechaComparar = new Date(estado.anio, estado.mes, dia);
    fechaComparar.setHours(0, 0, 0, 0);
    const esPasada = fechaComparar < hoy;
    const disponible = diasDisponibles.has(fecha) && !esPasada;
    const seleccionado = estado.fecha === fecha;
    ...
    <div
      class="${clases}"
      ${disponible ? `onclick="seleccionarFecha('${fecha}')"` : ""}
    >
      ${dia}
    </div>
  `;
...
function renderHoras(fechaStr) {
  ...
  const horas = estado.estilista.fechas[fechaStr] ?? [];
...
function seleccionarHora(hora) {
  ...
  if (window.ConfirmacionServicio) {
    localStorage.setItem("resumenReserva", JSON.stringify(estado));
    const servicio = JSON.parse(localStorage.getItem("servicioSeleccionado"));
    ...
    window.ConfirmacionServicio.actualizarServicio(
      nombreServicio,
      precioServicio,
    );
    window.ConfirmacionServicio.actualizarProfesional(
      estado.estilista.nombreEmpleado,
    );
